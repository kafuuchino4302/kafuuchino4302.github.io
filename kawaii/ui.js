...
export function displayResult({ rating, verdict: _ignored, explanation }) {
  elements.loading.classList.add('hidden');
  elements.result.classList.remove('hidden');

  const label = getRatingLabel(rating);
  const isHighScore = rating >= 80;
  elements.verdict.textContent = `${label} (${rating}/100)`;
  elements.verdictIcon.textContent = isHighScore ? '🌟' : '✨';
  elements.explanation.textContent = explanation;
  elements.result.className = `result ${isHighScore ? 'smash' : 'pass'}`; // 可保留原有样式逻辑
}
